...
3) When using CTRL+C / CTRL+D (keyboard interrupt) from main navigation menu or from any history menu, the application will exit immediately (same to what happens when entering ! + ENTER from the main menu). This is not the case for sub-menus like clipboard, recursive transfer or rename. In sub-menus, when hitting any of these interrupt key combinations, the app exits the sub-menu and returns to the main navigation page.