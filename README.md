...
It is possible to navigate to a specific favorite entry without accessing the menu by entering operator > followed by the entry number while in navigation mode (i.e. on main navigation page). This is a great way of speeding up the access if the user knows the entry number of the path to be visited "by heart". For example if directory /home/myUserName/Documents has entry number 2 in Favorites, then the user can enter >2 in navigation mode to visit it. If the string after the operator is not a valid entry number, then it will be considered a directory path and the script will attempt to visit it. If the path is invalid, an error will be triggered.

If you cannot find a entry simply enter the required path(s) to navigate to the directory you wish to visit. It is not required to exit the history and favorites menus in order to do this. Any input other than the given range of numbers or special options like the quit character (!) is considered regular navigation input. I call this the "input forwarding feature". This feature is present in other menus too.

The same rapid access mode can also be obtained with navigation history (by entering < followed by entry number) but in this case the quick history needs to be enabled. Also if the entry number is invalid (either not in range or containing invalid characters) an error will be triggered and the input will not be forwarded as regular navigation input. For more details, please check the quick history section (6.8).
